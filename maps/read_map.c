...
char	**parse_map2(t_game *game, int i)
{
	while (game->map[++i] != NULL)
	{
	...
	if (!(ft_check_line(game->map[i - 1], game->content.wall)))
		return (ft_free_map(game));
	if (game->map != NULL)
		game->copy_map = ft_clone(game->map);
	if (ft_checker_pathvalid(game) == 0)
	{
		ft_error("Error\n Path no valid\n");
...
char	**parse_map(int fd, t_game *game)
{
	...
	if (check_format(game) == 1)
	{
		ft_free_map(game);
		return (0);
	}
	ft_checker_content(game);
	if (check_symbols(game) == 1)
		return (ft_free_map(game));
	if (!(ft_check_line(game->map[0], game->content.wall)))
		return (ft_free_map(game));
	return (parse_map2(game, i));
}
